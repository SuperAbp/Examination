...
public class SqlServerProvider : ISqlProvider
{
    ...
    public string InsertPaper()
    {
        return @"INSERT INTO AppPapers (Id, Name, PaperType, Description, TotalQuestionCount, Score, CreationTime, TenantId, ExtraProperties, ConcurrencyStamp)
                 VALUES (@Id, @Name, @PaperType, @Description, @TotalQuestionCount, @Score, GETDATE(), @TenantId, '{}', REPLACE(CAST(NEWID() AS VARCHAR(36)), '-', ''))";
    }
    ...
    public string InsertPaperSections()
    {
        return @"INSERT INTO AppPaperSections (Id, PaperId, Title, ScoreEach, [Order], TotalScore, TotalCount, CreationTime, TenantId)
                 VALUES (@Id, @PaperId, @Title, @ScoreEach, @Order, @TotalScore, @TotalCount, GETDATE(), @TenantId)";
    }
    ...
    public string InsertPaperQuestionRules()
    {
        return @"INSERT INTO AppPaperQuestionRules (Id, PaperSectionId, QuestionBankId, QuestionType, Count, Score, CreationTime, TenantId)
                 VALUES (@Id, @PaperSectionId, @QuestionBankId, @QuestionType, @Count, @Score, GETDATE(), @TenantId)";
    }
    ...
    public string InsertPaperQuestions()
    {
        return @"INSERT INTO AppPaperQuestions (Id, PaperSectionId, QuestionId, Score, [Order], CreationTime, TenantId)
                 VALUES (@Id, @PaperSectionId, @QuestionId, @Score, @Order, GETDATE(), @TenantId)";
    }
    ...
    public string InsertExaminations()
    {
        return @"INSERT INTO AppExaminations (Id, Name, Description, Score, PassingScore, TotalTime, PaperId, Status,
                 ...
                 @RandomOrderOfOption, @StartTime, @EndTime, GETDATE(), @TenantId, '{}', REPLACE(CAST(NEWID() AS VARCHAR(36)), '-', ''))";
    }
    ...
    {
        return "INSERT INTO AppQuestionBanks (Id, Title, Remark, ExtraProperties, ConcurrencyStamp, CreationTime, TenantId) VALUES (@Id, @Title, @Remark, '{}', REPLACE(CAST(NEWID() AS VARCHAR(36)), '-', ''), @CreationTime, @TenantId)";
    }
    ...
    {
        return "INSERT INTO AppQuestionKnowledgePoints (QuestionId, KnowledgePointId, CreationTime, TenantId, ExtraProperties, ConcurrencyStamp) VALUES (@QuestionId, @KnowledgePointId, @CreationTime, @TenantId, '{}', REPLACE(CAST(NEWID() AS VARCHAR(36)), '-', ''))";
    }
}